...
delivery_van 是一个**周度发车管理工具**，按 `docs/周度发车机制设计方案.md` 实现。核心隐喻：团队每周五发一班"厢式快递车"，任务是快件，周五验收只看"这班的件送没送到"；没送完的滞留件跟下一班车走。

- 当前版本 v1.0，代号 `niulai`（大版本以动漫代号命名，仿 Vue 传统，谱系见 `README.md`）。
- 单页应用：一个看板页（`BoardPage`）承载全部功能——成员产能、任务大厅（委托）、周任务表、班次切换、结转与统计。
- **任务大厅**：UI 上的「任务大厅」（代码里仍叫 `pool_items`）存放的是**委托**（可接取的工作条目），不是需求；需求本身在团队另一套流程里管理。后续演进方向见 `docs/任务大厅与稀有度分级设计方案.md`。
- 无账号体系、无鉴权：小团队内部工具，任务负责人直接存姓名字符串。成员是**永久标签，不可删除**（没有删除入口与接口）。
- 代码与文档注释主要使用**中文**，新代码请沿用中文注释风格。

### 核心业务规则（改动代码时不得破坏）

- **班次编码**：仿期货合约风格 `DV` + 2 位年 + 2 位月 + 字母序号（如 `DV2607A`）。班次归属发车日（周五）所在月份，字母 = 当月第几个周五（A–E）。规则与测试在 `contracts/vans.ts`、`contracts/vans.test.ts`。
- **三档粒度**：任务体量只允许 1 / 3 / 5 天，接口层用 zod 强制（拒绝 2/4 天）。
- **运力校验**：每人每班档位合计 ≤ 当周运力（默认 5 天，请假扣减），超载由服务端直接拒单（`assertWithinCapacity`）；负责人不在成员表中时不校验。调低成员运力不得低于其在各班次的已排档位峰值（`updateMemberCapacity` 回检）。
- **送达二值化**：没有"完成 80%"。打勾 `done` 自动记送达日期（今天），取消完成自动清空日期；送达日期可手工补录/改填。
- **滞留结转**：未完成任务一键转下一班（只能转**紧邻的下一班**，服务端校验 `toVan === nextVanCode(fromVan)`），记录 `carriedFrom` 来源班次；`carryCount >= 2` 触发强制复盘**提示**（仅提示不拦截）；同一对班次幂等 + 事务包裹，防重复转运与半截数据。
- **任务大厅联动**：委托三态（open 待切片 / scheduled 已排期 / done 已完成），由关联任务自动推导（`syncPoolStatus`：无关联任务 → open，全部送达 → done，否则 scheduled）；手动编辑委托状态后，下一次任务增删改会重新推导覆盖。**Epic 委托不可直接上车**（`addTask`/`updateTask` 服务端拦截）。
...
contracts/    前后端共享代码
  vans.ts       班次编码工具（vanCodeOf / fridayOf / nextVanCode / 阶段判断等）
db/
  schema.ts     Drizzle 表定义：members（成员产能）、pool_items（任务大厅·委托）、tasks（周任务）
  seed.ts       种子脚本（仅示例成员），npm run db:seed 运行
src/          React 前端
  main.tsx      入口：BrowserRouter + TRPCProvider
  App.tsx       路由（仅看板页，path="*" 通配是有意的 SPA 回退）
  pages/BoardPage.tsx  看板主页面（AG Grid 表格 + tRPC 查询/变更）
...
docs/         《周度发车机制设计方案.md》——业务设计文档，理解规则先读它
...
npm test           # vitest run：班次编码、运力校验、委托状态推导、滞留件转运逻辑
...
npm run db:seed    # 写入示例成员（tsx db/seed.ts；注意需先启动过一次服务完成建表）
```
...
- 分层约定：router 只做 zod 校验与转发，业务逻辑与 SQL 写在 `api/queries/`；可纯函数化的逻辑（如 `assertWithinCapacity`、`toStrandedTask`、`poolStatusOf`）与 DB 访问分离，便于无库单测。
...
- 现有测试：`contracts/vans.test.ts`（班次编码规则）、`api/queries/van.test.ts`（运力校验、委托状态推导、滞留件转运纯函数）。
