...
- 当前版本 v1.2（行内拖拽排序），最新已发布产物 v1.2.0，代号 `niulai`（大版本以动漫代号命名，仿 Vue 传统，谱系见 `README.md`）。**代码已叠加 v2.0 Phase 1「博弈机制」**（签收制、链式审计日志、统计三件套、昨日天气、结转原因、徽章——见 `docs/doing/v2.0-博弈机制落地计划.md`），尚未发版。
