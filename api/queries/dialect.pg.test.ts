/* pg 方言变体（v2.2 任务 9）：TEST_PG_URL 存在时（CI service 容器）用真实
 * PostgreSQL 连接跑数据层行为套件 + 建表幂等冒烟；本地无连接串时整组 skip。
 * 注意：本文件不做 vi.mock，业务代码走真实 connection.ts（postgres.js 驱动）。
 * 隔离：每用例前 ensureSchema（幂等）+ 清表；容器库跨用例共享，用例不得假设
 * 自增 id 从 1 起。 */
import { restorePgTestEnv } from "./testEnv.pg"; // 第一顺位 import：先于业务模块设定方言环境
...
describe.skipIf(!TEST_PG_URL)("方言变体：postgres（CI 容器真实连接）", () => {
