...
- 没送完的件自动跟下一班车走（滞留结转），系统记录来源班次
- 连续两班都没送到？亮黄灯——是该拆得更小了，还是真有阻塞该升级了
- 任务大厅里的需求是**委托**（可以接取的工作条目），带稀有度分级，凭直觉定级
...
- **多人负责** 一个任务可由多人共同负责（标签输入），运力仅做记录不做校验；成员是永久标签，不可删除
- **滞留结转** 未完成一键转下一班（仅紧邻班次，目标班不存在时自动创建），事务 + 幂等防重；`carryCount >= 2` 触发强制复盘**提示**（仅提示不拦截）
- **任务大厅** 委托三态（待切片/已排期/已完成）由关联任务自动推导；手动编辑后下一次增删改会重新推导覆盖
- **稀有度** 六级标记（普通→神话），整体文字着色，神话彩虹动画；只是标记，系统不做拦截；待接取委托显示「挂 N 轮」
- **表头统计** 实时显示未送达 X / 共 Y 件、送达率、滞留率、稀有度构成
...
scripts/    跨平台生产启动
src/        React 前端（pages/BoardPage 看板、components/TagCellEditor 标签输入）
docs/       设计方案与发版计划
```
