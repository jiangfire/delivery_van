...
- **多人负责** 一个任务可由多人共同负责（勾选式多选），运力仅做记录不做校验；成员为**有守卫删除**：零历史成员（手滑加错的）可删，在任何快件上留过痕迹（负责人/提出人/签收人）的成员拒绝删除，保护统计与签收链路
...
- **三方来源（v2）** 每件标客户/平台/探索（默认客户），统计条速览迷你条 + 统计面板「来源」页签明细；v2.0 前历史件统一记客户件
- **昨日天气（v2）** 建议装载上限 = 上一班 done 点数合计，只提示不拦截
- **表头统计** 实时显示未送达 X / 共 Y 件、送达率、滞留率、强制复盘数、未签收、建议装载上限、徽章角标、日志指纹；稀有度构成占比在统计面板「稀有度」页签
